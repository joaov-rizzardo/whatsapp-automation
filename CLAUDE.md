...
**Four blocks execute** — `início`, `texto` (with `{{variáveis}}` and typing time), `aguardar`, `aguardar resposta` (with grouping of split messages). The other three (`comparação`, `definir variável`, `randomizar`) do not, and **that is what stops them from being published**: "has `execute`" and "is publishable" are the same sentence, asked of the same registry, so the restriction lifts by itself as each block gains its `execute` — no publishing code changes again.

Next: **the three remaining blocks** (which pull in the comparison engine with special types and time zone), then message types beyond text (one parser and one send block per type), and the screen that shows the conversations the bot had — which only now has data to display.
