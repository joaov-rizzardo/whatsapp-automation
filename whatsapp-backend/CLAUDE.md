...
src/
  ...
  modules/
    ...
    automations/                # the automation + its flow (spec 006)
      blocks/                   # the block registry: one file per block type
      blocks/comparison/        # the comparison engine (spec 009) — pure, and under
                                # blocks/ because the engine must not own it
...
- **`modules/automations/blocks/` is the block registry — and the place the execution engine will live.** One file per block type (`start`, `text`, `wait-reply`, `condition`, `set-variable`, `delay`, `randomizer`), each declaring `dataSchema` (JSON Schema, compiled once with Ajv at boot), `handles(data)`, and an optional `validate`. Adding a block type is one file here plus one on the frontend — **no migration, no column, no `if` in the service**. An unknown type is refused on save (`400 UNKNOWN_BLOCK_TYPE`), so nothing reaches the database in a shape the engine couldn't read.
- **Variables have two type families, and only one of them is persisted.** The document keeps `text | number | boolean`; `blocks/variable-types.ts` adds the **special** ones — `time | date | month | weekday` — which exist only on `blocks/system-variables.ts`. That split is why hora/data/mês/dia-da-semana cost no migration. Consequences the condition block enforces at publish: an operator carries `value: none | single | range | set` (the `range`/`set` shapes of `comparisonValueSchema`), a special value must parse as its type (`25:99` is refused), and a special variable can only be compared with a fixed value — never with another variable. Date and time are resolved in `America/Sao_Paulo`; `between` is inclusive and crosses midnight when the start is greater than the end.
...
Branching, writing and drawing lots are the fifth slice (spec 009), and the one that cost the least:

- **`blocks/comparison/` is the comparison engine, and it lives under `blocks/` — never under `flow-runtime/`.** Same dependency direction as the block contract: comparing is pure, and the next block that wants it ("wait until…") must not have to import the engine. `normalize.ts` (which `flow-runtime/keyword.ts` now imports, not the other way round), `comparable.ts` and `evaluate.ts`.
- **Almost every operator is an *ordering*.** `toComparable(type, value)` turns each type into something orderable — number, minutes since midnight, the ISO date string, the ISO weekday integer, normalized text — so `gt`/`lt`/`between`/`after`/`eq` are one piece of code, and only the genuinely specific parts are left (time's `between` crossing midnight, the set operators). Adding a type is one `case`.
- **An unreadable value makes the comparison false — including the negated operators.** `"dez" != "5"` is **false**, not true. Predictability over cleverness: "it didn't work out" leaves through the `false` handle the person drew for exactly that. Letting `neq` go true would make corrupt data *trigger* a branch.
- **Text compares normalized** (case, accent and punctuation don't count), because it is WhatsApp and `"Sim"`, `"sim."` and `"SIM!"` are one answer. `contains` is a **substring**, deliberately unlike the keyword trigger's whole-word rule — there the user had no choice, here they picked "contém" from a list that also offered "é". `empty` is the exception that looks at the **raw** value: an emoji-only message normalizes to empty and is not empty.
- **Two new ports on `RuntimeContext`, both for the same reason `now()` exists**: `variables.typeOf(id)` (a comparison's `data` carries the id and the operator and nothing else — only the store knows whether `"9" < "10"` is the text question or the number one) and `random()` (without it the randomizer's test is statistics, and a statistical test is a flaky one).
- **The `get` that had never been exercised.** `variables.ts` only resolved `sys:` inside `render`, by *name* — `get("sys:hora")` returned `""`, and no block until 009 read a system variable by id. All three of them do, constantly. It now routes any `sys:` id through the same computed values (`sys:ultima_resposta` excepted, the one system variable that is stored), and a write to a computed one is ignored and logged rather than throwing.
- The one validation rule the slice added: `increment`/`decrement` require a `number` variable, mirrored on both sides. Nothing else — no table, no migration, no env, no dependency, and the engine's `walk` untouched.
