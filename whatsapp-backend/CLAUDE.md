...
- `config/env.ts` (Zod, crashes at boot on a bad env), `plugins/` (`prisma`, `auth`, `require-auth`, `error-handler`), `shared/errors.ts`, `lib/auth.ts`.
- Better Auth serves `/api/auth/*`; `modules/me/` is a protected `GET /api/me` and the reference for the route shape.
- Prisma 7 + PostgreSQL with the `add_auth` migration; Vitest against the real database (`npm test`).
...
- **Do not wrap it in an `auth.service.ts` / `auth.repository.ts`.** A pass-through wrapper adds nothing and drifts from the library. Better Auth *is* the auth layer.
- **The `user`, `session`, `account` and `verification` tables belong to Better Auth.** They are generated by its CLI (`npx auth@<version> generate --adapter prisma`) — never hand-edited. No feature module writes to them; reading (`user.name` for display) from a feature's own repository is fine.
...
The CLI ships as the `auth` package (version-matched to `better-auth`), not `@better-auth/cli`, which lags well behind.
