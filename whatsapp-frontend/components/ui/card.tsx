...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        // ZapBot: raio 16px, borda 1px sutil, sombra difusa, padding generoso
        // (24px; 16px no size=sm). Cards nunca "apertados", sem header colorido.
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-lg border border-border bg-card py-(--card-spacing) text-base text-card-foreground shadow-sm [--card-spacing:--spacing(6)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(4)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-lg *:[img:last-child]:rounded-b-lg",
        className
      )}
      {...props}
    />
